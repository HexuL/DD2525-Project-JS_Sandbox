


**1.Principle of the vulnerability:** 

This vulnerability exists in vm module. This vulnerability enables an attacker to commit RCE attack from vm by constructor chain.



**2.How to use this package?**

You only need to install the latest package using `npm i constructor_chain_quan`, then import and execute it using `require("constructor_chain_quan");`.

**3.Output and security:**

This npm package is created solely for studying sandbox escape techniques. In console, it will print expected output and the real output.

**4.Code Source:**
https://gist.github.com/jcreedcmu/4f6e6d4a649405a9c86bb076905696af